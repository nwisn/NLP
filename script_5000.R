...
# define corpus path
cname <- file.path("/home", "ubuntu", "volume", "NLP", "gutenberg_5000")
...
# remove sparse terms
dtms.stem <- removeSparseTerms(dtm.stem, .5)
...
pdf("philosphy_wordcloud.pdf", height = 6, width = 6)
set.seed(120)
wordcloud(names(freq), freq, min.freq=freq[100], colors=brewer.pal(6, "Dark2"))
dev.off()

dtm_tfidf_stem <- DocumentTermMatrix(docs.stem, control = list(weighting = weightTfIdf))
dtms_tfidf_stem <- removeSparseTerms(dtm_tfidf_stem, .7)
...
taste.scores <- lapply(original.lemmastems, function(x) tm_term_score(dtms.stem, terms = x))
...
require(ggrepel)
pdf("philosphy_biplot.pdf", height = 10, width = 10)
ggbiplot(pca,
...
pdf("philosophy_heatmap.pdf", height = 8, width = 10, onefile = F)
